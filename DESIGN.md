...
Latency is the fourth golden signal. It is not a single number but a set of per-stage distributions. Because durations cannot be summed into a meaningful counter or sampled into a meaningful gauge, each stage is a `LatencyStage` exposing a running **sample `Count`** and **`SumNanos`** (summed duration in nanoseconds). An exporter derives the mean over any interval as `Δ(SumNanos)/Δ(Count)` — the same count/sum style as `Totals`, so the values are start-relative counters to be graphed as deltas (e.g. Graphite `nonNegativeDerivative`), not absolute levels. The three instance-wide stages live in `Stats.Latency`:
...
A fourth stage covers **client↔hub transit** and is reported **per hub connection** rather than instance-wide, as `Federation.Clients[].PublishRTT` (also a `LatencyStage`):

- **`PublishRTT`** — the round-trip time from a federation client sending a `PublishBatch` to receiving the hub's `PublishAck`, measured entirely on the client's own clock so it is free of cross-host clock skew. The client's send loop keeps only one batch in flight, so each RTT is unambiguously attributed to its batch; one sample per batch. Recorded only on a successful ack (a stalled or failed batch contributes nothing), and accumulated on the `Client` so it survives reconnects. This covers the regular federation `Client`; the ephemeral client is not yet instrumented. It measures send→ack, which includes the hub's own write — it is transit *plus* remote durability, not pure network time.

The remaining gap is **fixed-bucket histograms** per stage for percentiles/tails (the count+sum mean hides a p99 spike), planned next. Histograms are additive over the existing `LatencyStage` fields, so adding them will not change the current API.
