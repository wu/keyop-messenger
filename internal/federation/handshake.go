package federation

import (
	"encoding/json"
	"fmt"
)

// HandshakeMsg is exchanged as the first message on every new connection,
// by both hub peers and clients.
type HandshakeMsg struct {
	InstanceName string   `json:"instance_name"`
	Role         string   `json:"role"`                // "hub" or "client"
	Version      string   `json:"version"`             // wire protocol version, currently "1"
	LastID       string   `json:"last_id,omitempty"`   // last acked ID; non-empty on reconnect
	Subscribe    []string `json:"subscribe,omitempty"` // channels the client wants to receive
	// Ephemeral signals that this client does not maintain local state.
	// The hub must not replay missed messages even if LastID is set.
	Ephemeral bool `json:"ephemeral,omitempty"`
}

// AckMsg is sent by the receiver after processing a batch of data frames.
// LastID is the envelope ID of the last successfully processed message,
// used to drive replay on reconnect.
type AckMsg struct {
	LastID string `json:"last_id"`
}

// SendHandshake writes msg to conn as a JSON-encoded message.
func SendHandshake(conn *Conn, msg HandshakeMsg) error {
	data, err := json.Marshal(msg)
	if err != nil {
		return fmt.Errorf("federation: marshal handshake: %w", err)
	}
	if err := conn.WriteMessage(MsgTypeJSON, data); err != nil {
		return fmt.Errorf("federation: send handshake: %w", err)
	}
	return nil
}

// ReceiveHandshake reads the next message from conn and decodes it as a
// HandshakeMsg. Returns an error if the frame is not a JSON message.
func ReceiveHandshake(conn *Conn) (HandshakeMsg, error) {
	var msg HandshakeMsg
	msgType, data, err := conn.ReadMessage()
	if err != nil {
		return msg, fmt.Errorf("federation: receive handshake: %w", err)
	}
	if msgType != MsgTypeJSON {
		return msg, fmt.Errorf("federation: handshake: expected JSON frame, got type %d", msgType)
	}
	if err := json.Unmarshal(data, &msg); err != nil {
		return msg, fmt.Errorf("federation: unmarshal handshake: %w", err)
	}
	return msg, nil
}

// SendAck writes ack to conn as a JSON-encoded message.
func SendAck(conn *Conn, ack AckMsg) error {
	data, err := json.Marshal(ack)
	if err != nil {
		return fmt.Errorf("federation: marshal ack: %w", err)
	}
	if err := conn.WriteMessage(MsgTypeJSON, data); err != nil {
		return fmt.Errorf("federation: send ack: %w", err)
	}
	return nil
}

// ReceiveAck reads the next message from conn and decodes it as an AckMsg.
// Returns an error if the frame is not a JSON message.
func ReceiveAck(conn *Conn) (AckMsg, error) {
	var ack AckMsg
	msgType, data, err := conn.ReadMessage()
	if err != nil {
		return ack, fmt.Errorf("federation: receive ack: %w", err)
	}
	if msgType != MsgTypeJSON {
		return ack, fmt.Errorf("federation: ack: expected JSON frame, got type %d", msgType)
	}
	if err := json.Unmarshal(data, &ack); err != nil {
		return ack, fmt.Errorf("federation: unmarshal ack: %w", err)
	}
	return ack, nil
}
