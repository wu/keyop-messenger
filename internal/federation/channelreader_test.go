...
// committedAll bounds a reader at the channel's current stream end — correct for
...
func committedAll(layout storage.Layout, channel string) func() int64 {
	return func() int64 {
		end, err := storage.ChannelStreamEnd(layout.ChannelDir(channel))
		if err != nil {
			return 0
		}
		return end
...
func TestNewChannelReader_SanitizesPeerNameForOffsetPath(t *testing.T) {
	...
	_, err := newChannelReader(layout, "../../evil", "ch", "fed-", 65536, requestCh,
		nil, committedAll(layout, "ch"), log)
	require.NoError(t, err)
...
func TestChannelReader_SkipsOversizedRecord(t *testing.T) {
	...
	cr, err := newChannelReader(layout, "peer1", "big", "fed-", 2*1024, requestCh,
		nil, committedAll(layout, "big"), log)
	require.NoError(t, err)
...
func TestChannelReader_SkipsUnscannableRecord(t *testing.T) {
	...
	requestCh := make(chan sendReq, 4)
	cr, err := newChannelReader(layout, "peer1", "huge", "fed-", maxBatch, requestCh,
		nil, committedAll(layout, "huge"), log)
	require.NoError(t, err)
...
func TestChannelReader_SkipsLoneOversizedRecord(t *testing.T) {
	...
	requestCh := make(chan sendReq, 4)
	cr, err := newChannelReader(layout, "peer1", "lone", "fed-", 2*1024, requestCh,
		nil, committedAll(layout, "lone"), log)
	require.NoError(t, err)
...
func TestChannelReader_NewSubscriber_StartsAtEnd(t *testing.T) {
	...
	requestCh := make(chan sendReq, 4)
	cr, err := newChannelReader(layout, "peer1", "test-ch", "fed-", 65536, requestCh,
		nil, committedAll(layout, "test-ch"), log)
	require.NoError(t, err)
...
func TestChannelReader_DropsEchoSendSide(t *testing.T) {
	...
	cr, err := newChannelReader(layout, "peer-hub-a", "metrics", storage.OffsetPrefixFedOut,
		65536, requestCh, func() string { return "hub-a" }, committedAll(layout, "metrics"), log)
	require.NoError(t, err)
...
func TestChannelReader_DeliveryAndOffsetPersistence(t *testing.T) {
	...
	cr, err := newChannelReader(layout, "peer1", "events", "fed-", 65536, requestCh,
		nil, committedAll(layout, "events"), log)
	require.NoError(t, err)
...
func TestChannelReader_BatchSizeLimit(t *testing.T) {
	...
	cr, err := newChannelReader(layout, "peer1", "big", "fed-", 0 /* placeholder */, requestCh,
		nil, committedAll(layout, "big"), log)
	require.NoError(t, err)
...
func TestChannelReader_NotificationWakeup(t *testing.T) {
	...
	requestCh := make(chan sendReq, 4)
	cr, err := newChannelReader(layout, "peer1", "wake", "fed-", 65536, requestCh,
		nil, committedAll(layout, "wake"), log)
	require.NoError(t, err)
	cr.start()
	t.Cleanup(cr.close)
...
func TestChannelReader_ResumeFromOffset(t *testing.T) {
	...
	requestCh := make(chan sendReq, 4)
	cr, err := newChannelReader(layout, "peer1", "resume", "fed-", 65536, requestCh,
		nil, committedAll(layout, "resume"), log)
	require.NoError(t, err)
	cr.start()
	t.Cleanup(cr.close)
	cr.notify()
...
func TestChannelReader_UndercutResumesAtOldestAvailable(t *testing.T) {
	...
	requestCh := make(chan sendReq, 4)
	cr, err := newChannelReader(layout, "peer1", "undercut", "fed-", 65536, requestCh,
		nil, committedAll(layout, "undercut"), log)
	require.NoError(t, err)
	cr.start()
	t.Cleanup(cr.close)
	cr.notify()
...
func TestChannelReader_MultipleNotificationsCoalesced(t *testing.T) {
	...
	requestCh := make(chan sendReq, 4)
	cr, err := newChannelReader(layout, "peer1", "coalesce", "fed-", 65536, requestCh,
		nil, committedAll(layout, "coalesce"), log)
	require.NoError(t, err)
...
func TestChannelReader_Close_StopsGoroutine(t *testing.T) {
	...
	cr, err := newChannelReader(layout, "peer1", "stopchan", "fed-", 65536, requestCh,
		nil, committedAll(layout, "stopchan"), log)
	require.NoError(t, err)
...
func TestChannelReader_ConcurrentNotify(t *testing.T) {
	...
	cr, err := newChannelReader(layout, "peer1", "concurrent", "fed-", 65536, requestCh,
		nil, committedAll(layout, "concurrent"), log)
	require.NoError(t, err)
	cr.start()
	t.Cleanup(cr.close)
...
// TestChannelReader_InFlightRecordIsUnreachable is the regression test for the
// 2026-09-11 incident, restated against the property that now prevents it.
//
// Then: the reader tailed a segment to EOF, so it saw the bytes of a record the
// writer was still appending, framed them as a record, logged "unmarshal corrupt
// record", advanced past them, and resumed mid-record — a lost message followed
// by a mis-framed stream.
//
// Now: those bytes are outside the readable region. The reader stops at the
// committed end, which sits on the last complete record, and delivers the
// in-flight record whole once the writer commits it.
func TestChannelReader_InFlightRecordIsUnreachable(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
	layout := storage.NewLayout(dir)
	channelDir := layout.ChannelDir("metrics")
	...
	// One committed record, then half of the next still being written.
	require.NoError(t, os.MkdirAll(channelDir, 0o750))
	...
	requestCh := make(chan sendReq, 4)
	cr, err := newChannelReader(layout, "peer1", "metrics", storage.OffsetPrefixFedIn,
		65536, requestCh, nil, end.Load, log)
	require.NoError(t, err)
	cr.offset = 0
	...
	select {
	case req := <-requestCh:
		require.Len(t, req.rawLines, 1, "only the committed record is readable")
		assert.Equal(t, completeBytes, req.rawLines[0])
		assert.Equal(t, boundary, req.newOffset, "the offset stops at the record boundary")
		close(req.doneCh)
	case <-time.After(5 * time.Second):
		t.Fatal("timed out waiting for the committed record")
	}

	// The writer finishes the record and commits it.
	...
	end.Store(boundary + int64(len(inflightBytes)+1))
	cr.notify()

	select {
	case req := <-requestCh:
		require.Len(t, req.rawLines, 1)
		assert.Equal(t, inflightBytes, req.rawLines[0], "delivered intact once committed")
		close(req.doneCh)
	case <-time.After(5 * time.Second):
		t.Fatal("timed out waiting for the newly committed record")
	}
	assert.False(t, log.HasError("unmarshal corrupt record"),
		"no record may be mis-framed at any point")
}
...
func TestChannelReader_CorruptRecordLogsDiagnostics(t *testing.T) {
	...
	requestCh := make(chan sendReq, 4)
	cr, err := newChannelReader(layout, "peer1", "metrics", "fed-", 65536, requestCh,
		nil, committedAll(layout, "metrics"), log)
	require.NoError(t, err)
...
// TestChannelReader_BoundedByCommittedEnd verifies that a reader given a
// committed end never reads past it, even when the file on disk is longer —
// which is the state during every in-flight write. The bytes past the bound are
// not skipped, only deferred: once the writer commits them they are delivered.
func TestChannelReader_BoundedByCommittedEnd(t *testing.T) {
	...
	second := makeEnvelope(t, "metrics", "second")
	firstBytes, err := envelope.Marshal(first)
	require.NoError(t, err)
	secondBytes, err := envelope.Marshal(second)
	require.NoError(t, err)

	// Both records are fully on disk, but only the first is committed.
	data := writeTestSegment(t, channelDir, 0, []envelope.Envelope{first, second})
	committed := int64(len(firstBytes) + 1)
	var end atomic.Int64
	end.Store(committed)
	...
	case req := <-requestCh:
		require.Len(t, req.rawLines, 1, "only the committed record is readable")
		assert.Equal(t, firstBytes, req.rawLines[0])
		assert.Equal(t, committed, req.newOffset)
		close(req.doneCh)
	case <-time.After(5 * time.Second):
		t.Fatal("timed out waiting for the committed record")
	}

	// Commit the rest; the deferred record must now be delivered intact.
	end.Store(int64(len(data)))
	cr.notify()

	select {
	case req := <-requestCh:
		require.Len(t, req.rawLines, 1)
		assert.Equal(t, secondBytes, req.rawLines[0])
		assert.Equal(t, int64(len(data)), req.newOffset)
		close(req.doneCh)
	case <-time.After(5 * time.Second):
		t.Fatal("timed out waiting for the newly committed record")
...
// TestChannelReader_NothingCommittedDeliversNothing pins the contract that
// replaced the old "unknown committed end → read to EOF" fallback: a reader
// delivers only what the writer has confirmed, and a channel with nothing
// committed has nothing to deliver.
func TestChannelReader_NothingCommittedDeliversNothing(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
	layout := storage.NewLayout(dir)
	log := &testutil.FakeLogger{}
	...
	env2 := makeEnvelope(t, "metrics", "two")
	writeTestSegment(t, layout.ChannelDir("metrics"), 0, []envelope.Envelope{env1, env2})

	requestCh := make(chan sendReq, 4)
	cr, err := newChannelReader(layout, "peer1", "metrics", storage.OffsetPrefixFedIn, 65536,
	...
	case req := <-requestCh:
		close(req.doneCh)
		t.Fatalf("delivered %d uncommitted records", len(req.rawLines))
